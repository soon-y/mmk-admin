...
import MMKColor from "../asset/mmk"
...
export function Palette({ initial, index, setColorArray, setColorHex }: {
  ...
  const palette: string[] = ['#ffffff', '#000000', '#e3007f', '#00a0e8', '#ffef00', 'MMK']
  const colors: string[] = ['White', 'Black', 'Magenta', 'Blue', 'Yellow', 'Mixed']
  ...
  return (
    <div className="flex gap-1 cursor-pointer">
      ...
      <div onClick={() => setColorIndex(5)} className={`${colorIndex === 5 ? 'border-gray-700' : 'border-gray-300 opacity-50'} rounded-full border border-2 w-5 h-5 overflow-hidden bg-white"`}>
        <MMKColor />
      </div>
    </div>
  )
